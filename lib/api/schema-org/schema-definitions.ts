...
export const NULL = z.literal(null).describe('Null value').meta({ id: 'NULL' });

// Utility function for optional fields in Vercel AI SDK
function optional<T>(schema: z.ZodType<T>) {
  return z.union([schema, NULL]);
}
...
const OrganizationBaseSchema = z
  .object({
    ...
    name: z.string(),
    url: optional(z.string()),
    logo: optional(z.union([z.string(), ImageObjectSchema])),
  })
...
const OfferSchema = z
  .object({
    ...
    price: optional(z.union([z.string(), z.number()])),
    priceCurrency: optional(
      z.string().describe('ISO 4217 currency code (e.g., USD, EUR)')
    ),
    ...
    validFrom: optional(
      z.string().describe('ISO 8601 date when the offer becomes valid')
    ),
    priceValidUntil: optional(
      z.string().describe('ISO 8601 date when the price expires')
    ),
...
const AggregateRatingSchema = z
  .object({
    ...
    reviewCount: optional(z.union([z.string(), z.number()])),
    ratingCount: optional(z.union([z.string(), z.number()])),
    bestRating: optional(
      z
        .union([z.string(), z.number()])
        .describe('Best possible rating (default is 5)')
...
const PlaceSchema = z
  .object({
    ...
    address: optional(z.union([z.string(), PostalAddressSchema])),
...
const NutritionInformationSchema = z
  .object({
    ...
    calories: optional(
      z.string().describe('Calorie count (e.g., "250 calories")')
    ),
  })
...
const ContactPointSchema = z
  .object({
    ...
    telephone: optional(
      z.string().describe('Phone number in international format')
    ),
    contactType: optional(
      z.string().describe('Type of contact (e.g., customer service, sales)')
    ),
...
export const EventSchema = z
  .object({
    ...
    endDate: optional(
      z.string().describe('ISO 8601 date-time when the event ends')
    ),
    ...
    location: optional(z.union([PlaceSchema, VirtualLocationSchema])),
    image: optional(z.union([z.string(), z.array(z.string())])),
    ...
    offers: optional(z.union([OfferSchema, z.array(OfferSchema)])),
  })
  .meta({ id: 'EventSchema' });
...
export const OrganizationSchema = z
  .object({
    ...
    contactPoint: optional(
      z.union([ContactPointSchema, z.array(ContactPointSchema)])
    ),
    sameAs: optional(
      z
        .union([z.string(), z.array(z.string())])
    ...
    foundingDate: optional(
      z.string().describe('ISO 8601 date when the organization was founded')
    ),
    founders: optional(z.union([PersonSchema, z.array(PersonSchema)])),
  })
...
export const ArticleSchema = z
  .object({
    ...
    datePublished: optional(
      z.string().describe('ISO 8601 date when the article was first published')
    ),
    dateModified: optional(
      z.string().describe('ISO 8601 date when the article was last modified')
    ),
    description: optional(z.string()),
    articleBody: optional(
      z.string().describe('Full text content of the article')
    ),
  })
...
export const ProductSchema = z
  .object({
    ...
    description: optional(z.string()),
    sku: optional(
      z.string().describe('Stock Keeping Unit - unique product identifier')
    ),
    mpn: optional(z.string().describe('Manufacturer Part Number')),
    brand: optional(z.union([BrandSchema, z.string()])),
    offers: optional(z.union([OfferSchema, z.array(OfferSchema)])),
  ...
  .object({
    ...
    itemListElement: z.array(
      z.object({
        ...
        name: z.string(),
        item: optional(
          z.string().describe('URL of the page (omit for the current page)')
        ),
      })
    ),
  })
...
export const RecipeSchema = z
  .object({
    ...
    datePublished: optional(
      z.string().describe('ISO 8601 date when the recipe was published')
    ),
    description: optional(z.string()),
    prepTime: optional(
      z
    ...
    recipeYield: optional(
      z
        .union([z.string(), z.number()])
        .describe(
          'Number of servings the recipe makes (e.g., "4 servings" or 4)'
        )
    ),
    recipeCategory: optional(
      z
        .string()
    ...
    keywords: optional(
      z.string().describe('Comma-separated keywords about the recipe')
    ),
...
export const VideoObjectSchema = z
  .object({
    ...
    name: z.string(),
    description: optional(z.string()),
    thumbnailUrl: optional(z.union([z.string(), z.array(z.string())])),
    uploadDate: optional(
      z.string().describe('ISO 8601 date when the video was uploaded')
    ),
    duration: optional(
      z
        .string()
...
export const ReviewSchema = z
  .object({
    ...
    datePublished: optional(
      z.string().describe('ISO 8601 date when the review was published')
    ),
    publisher: optional(OrganizationBaseSchema),
  })
  ...
  .object({
    ...
    offers: optional(
      z.object({
        '@type': z.literal('Offer'),
        category: optional(
          z.string().describe('Category of the offer (e.g., Paid, Free)')
        ),
        price: optional(z.union([z.string(), z.number()])),
        priceCurrency: optional(
          z.string().describe('ISO 4217 currency code (e.g., USD)')
        ),
      })
    ),
          ...
          z.object({
            ...
            courseMode: optional(
              z
                .string()
                .describe(
                  'Mode of instruction (e.g., online, in-person, blended)'
                )
            ),
            startDate: optional(
              z.string().describe('ISO 8601 date when this instance starts')
            ),
            endDate: optional(
              z.string().describe('ISO 8601 date when this instance ends')
            ),
...
export const JobPostingSchema = z
  .object({
    ...
    validThrough: optional(
      z.string().describe('ISO 8601 date when the job posting expires')
    ),
    ...
    jobLocation: optional(z.union([PlaceSchema, z.array(PlaceSchema)])),
