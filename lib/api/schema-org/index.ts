...
function schemaItem(schema: z.ZodObject, id: string) {
    return z.object({
        ...
        probabilityExplanation: z.string()
            .describe(`Short but detailed explanation of probability percent of '${id}' schema suits page (1-2 sentences)`),
        item: z.union([
            NULL,
            schema,
        ]).describe(`Set null if probability is low, populate with '${id}' schema if probability is high`).meta({ id }),
    });
}
